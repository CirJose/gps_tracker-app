...
function DataTable({
  ...
  // Para alertas
  const showMsg = useSelector(state => { return state.rolReducer.showMsg }, shallowEqual);
  ...
  useEffect(() => {
    dispatch(actions.requestListaRol({}));
  }, []);
  ...
  useEffect(() => {
    ...
    if (!rolReducer.isChanging) {
      setRows(rolReducer.listRol);
      // let updatedData = data.concat(rolReducer.listUsers);
      // setData(updatedData);
      // setRefreshing(false);
    }
  ...
  useEffect(() => {
    // Verifica si está ejecutando la petición en axios
    if (showMsg) {
      if (rolReducer.isError) {
        setOpenMsg(true);
        setMsg({
          severity: 'error',
          text: rolReducer.message
        })
      } else {
        setOpenMsg(true);
        setMsg({
  ...
  const handleDelete = () => {
    setRows([]);
    dispatch(actions.requestDeleteRol(elemento));
    setOpen(false);
  }

  const handleDetails = (data = {}) => {
    dispatch(detailsActions.setearRol(data));
    navigate("/rol/details");
  }
