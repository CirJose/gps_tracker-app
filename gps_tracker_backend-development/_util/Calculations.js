...
const Redis = require("ioredis"); // Usamos ioredis para almacenar las consultas 

const redis = new Redis(); // Automáticamente se conecta a 127.0.0.1:6379, hay que tener configurado la BD de Redis
const config = {};
...
async function getGatewaysByMacs(macs) {
  ...
  const redisPromises = macs.map(mac => redis.get(mac));
  const redisData = await Promise.all(redisPromises);

  const macsNotFoundInRedis = [];

  // Guarda los datos que se encuentran en Redis en el objeto de resultado
  for (let i = 0; i < macs.length; i++) {
    if (redisData[i]) {
      result[macs[i]] = JSON.parse(redisData[i]);
    } else {
      macsNotFoundInRedis.push(macs[i]);
    }
  }

  // Si hay macs que no se encontraron en Redis, búscalos en la base de datos
  if (macsNotFoundInRedis.length > 0) {
    const gateways = await ibeaconGateway.findAll({
      where: {
        mac: macsNotFoundInRedis
      },
    ...
    for (const gateway of gateways) {
      ...
      result[gateway.mac] = gatewayData;

      // Guarda los datos en Redis para futuras consultas
      await redis.set(gateway.mac, JSON.stringify(gatewayData));
    }
  }
